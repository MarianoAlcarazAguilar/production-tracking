...
class ExcelFunctions:
    ...
    def get_columns_on_row(self, working_sheet, row, num_nones_consecutivos=3, starting_col=1):
        ...
        for columna, celda in enumerate(working_sheet[row], start=starting_col):
            ...
            if valor_celda is not None:
                diccionario_columnas[valor_celda] = columna - starting_col + 1
                nones_consecutivos = 0
            else:
    ...
    def find_value_with_pandas(self, file_location:str, value_to_find) -> list:
        '''
        ...
        result = (
            pd
            .read_excel(file_location, skiprows=0, header=None)
            .rename(lambda x: x+1)
    ...
    def extract_data_from_file(self, file, index_value, cols_to_extract:list, shift_between_values:int=0, sheet_name=0) -> pd.DataFrame:
        '''
        ...
        Inputs:
            - file: ubicación del archivo excel
        ...
        dic_cells = {llave:self.find_value_with_pandas(file, llave)[0] for llave in cols_to_extract}
    ...
    def split_excel_sheets_to_files(self, source_excel, target_folder, folder_name):
        ...
        for sheet_name in xls.sheet_names:
            # Definir el nombre del archivo destino
            output_file = os.path.join(full_path, f"{sheet_name}.xlsx")
            ...
            if not os.path.isfile(output_file):
                ...
                # Guardar la hoja en un nuevo archivo Excel
                df.to_excel(output_file, index=False)

        print(f"Archivos creados en la carpeta: {full_path}")
